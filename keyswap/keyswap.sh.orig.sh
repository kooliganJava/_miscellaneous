...
usage() {
  echo "keyswap.sh <hostname> -- create new local ssh keys and update <hostname>"
  exit 0
}
...
startswap() {
  ...
  if [ $? -ne 0 ] ; then
    ...
    scp /usr/bin/keyswap.sh $remotehost:/usr/bin
    if [ $? -ne 0 ] ; then
      echo "can't copy keyswap.sh to remote host, trying to sync anyway..."
    fi
    errmsg=`ssh $remotehost "keyswap.sh -check 2> /dev/null"`
    if [ $? -ne 0 ] ; then
...
#      grep -v "sccp,127.2.0.1" /tmp/newsshkeys/known_hosts >> /var/ssh/root/known_hosts
  ...
  if [ $onlylocal -eq 0 ] ; then
    ...
    remotetmpdir=`ssh $remotehost "keyswap.sh -mktmpdir"`
    ...
    errmsg=`ssh $remotehost "keyswap.sh -checkkeys ${remotetmpdir}"`
    if [ $? -ne 0 ] ; then echo "can't ssh to $remotehost to check dir(${errmsg})"; return 1; fi

    # now commit the keys
    errmsg=`ssh $remotehost "keyswap.sh -commitkeys ${remotetmpdir}"`
